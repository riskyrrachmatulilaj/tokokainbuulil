...
<head>
    <meta charset="utf-8">
    <title>Rincian Piutang - {{ $party->name }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .brand h1 { margin: 0; font-size: 18px; color: #059669; }
...
<body>
    ...
    <div class="customer-box">
        ...
        <div class="info">
            <span>Telepon:</span> {{ $party->phone ?: '-' }} &nbsp;|&nbsp;
            <span>Alamat:</span> {{ $party->address ?: '-' }}
        </div>
    </div>
    ...
    <h2>Daftar Nota &amp; Rincian Penerimaan</h2>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @if ($receivable->description)
                            <br><small style="color:#6b7280;">{{ $receivable->description }}</small>
                        @endif
                    </td>
                    <td>{{ $receivable->receivable_date?->format('d M Y') }}</td>
                    <td>{{ $receivable->due_date?->format('d M Y') ?: '-' }}</td>
                    <td class="right">{{ number_format($receivable->amount, 0, ',', '.') }}</td>
                    <td class="right">{{ number_format($receivable->paid_amount, 0, ',', '.') }}</td>
                    <td class="right"><strong>{{ number_format($receivable->remaining_amount, 0, ',', '.') }}</strong></td>
                    <td>
                        <span class="badge {{ $receivable->status === 'paid' ? 'badge-success' : 'badge-danger' }}">{{ $receivable->status_label }}</span>
                    </td>
                </tr>
                @if ($receivable->paymentHistories->isNotEmpty())
                    <tr>
                        <td colspan="8" style="padding:0;">
                            <div class="payments">
                                <div class="p-title">RINCIAN PENERIMAAN</div>
                                <table>
                                    <thead>
                                        <tr>
                                            <th>Tanggal</th>
                                    ...
                                    <tbody>
                                        @foreach ($receivable->paymentHistories as $payment)
                                            <tr>
            ...
            @empty
                <tr>
                    <td colspan="8" style="text-align:center;padding:16px;color:#6b7280;">
                        Debitur tidak memiliki nota piutang.
                    </td>
                </tr>
            @endforelse
        </tbody>

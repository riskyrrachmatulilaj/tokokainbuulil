...
namespace App\Filament\Resources\ReceivableCollectivePaymentResource\RelationManagers;
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('amount')
                    ...
                    ->sortable(),
                Tables\Columns\TextColumn::make('receivable.receivable_date')
                    ->label('Tanggal Nota')
                    ->date('d M Y')
                    ->sortable(),
                Tables\Columns\TextColumn::make('receivable.status')
                    ->label('Status Nota')
                    ->badge()
                    ->color(fn ($state) => $state === 'paid' ? 'success' : 'danger')
                    ->formatStateUsing(fn ($state) => $state === 'paid' ? 'Lunas' : 'Belum Lunas'),
            ])
            ->defaultSort('receivable.receivable_date');
    }
}
